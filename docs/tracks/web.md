...
- 本轮交接目录：`90-Archive/StockWatcher/99-deliveries/StockWatcher-Web-Top3-Gain-20260812/`。
...
1. 当前 macOS worktree 的完整 pytest 为 `499 passed, 25 skipped, 2 deselected`，
   ...
   （29 个必需文件）和 JavaScript syntax check 全部通过。生产与全锁定依赖审计无已知漏洞。
2. Web、唯一 Worker、Caddy/Tunnel 当前在 Mac Docker Desktop 运行；镜像
   `stockwatcher-web:web-alpha4-cfc6cd6`、Schema v9、公网 `app.css?v=13`、部署前后备份和
   公网 live/ready 均已验证。
...
- 认证只读 outcomes API、近一月摘要/完整页、Morandi 视觉与中央自动提醒。
- Top3 上涨百分比使用旧版鲜明红并响应式放大；下跌与其他界面继续 Morandi 色系。
...
1. 在下一真实交易日验收 09:45/14:45 中央提醒、次日同点结算/有界重试、
   强异动与全日持续运行。
2. 浏览器完全关闭后的 Web Push 如仍需要，作为非阻断的独立后续功能。
3. 现场门通过后，由 Human Owner 决定是否建立受控集成提交；在此之前保持
   branch/worktree/交接包完整并继续 `BLOCKED / NOT_ACCEPTED`。
