...
| Web | 独立线 `feat/web-outcomes-morandi-alerts@8722814`；部署镜像源码 `d2dfc90` / SQLite v9 | `499 passed, 25 skipped, 2 deselected`；Ruff/Mypy/validator/JS、v8→v9 迁移、备份校验、Morandi UI 与中央提醒离线交互全绿；Mac Docker 与公网 live/ready HTTP 200 | 真实交易日 09:45/14:45、登录后手动抓取及用户现场验收；浏览器完全关闭后的 Web Push 未实现 | **`BLOCKED / NOT_ACCEPTED`** |
