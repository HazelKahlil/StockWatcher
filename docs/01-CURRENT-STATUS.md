...
| --- | --- | --- | --- | --- |
| 次日同点复盘 | `main@6a81825` / Web 可恢复节点 `b061269`，开发版本 `0.6.0a4`；桌面 SQLite v8 / Web v9 | Web 离线 `509 passed, 26 skipped`，`-W error` 零告警；严格日历、最多五次重试、认证 outcomes API 与迁移门全绿 | 真实下一交易日 09:45/14:45 同点结算仍待现场验收 | `local_code_complete_offline_verified_after_alpha4_reliability_fix` |
...
| Web | `fix/web-desktop-ui-alert-lifecycle`，基于 `b061269`，本轮未部署 | `509 passed, 26 skipped`、`-W error` 零告警、定向 Web 契约 `35 passed`；本机 Chrome 已验证桌面布局、右下角关闭、刷新/重开不重放旧提醒；Schema v9 | 本轮部署，以及真实交易日 09:45/14:45 固定提醒、同点结算/重试与全日运行现场验收 | **`BLOCKED / NOT_ACCEPTED`** |
