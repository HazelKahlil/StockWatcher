...
| --- | --- | --- | --- | --- |
| 次日同点复盘 | `main@b00221f` / Web `cfc6cd6`，开发版本 `0.6.0a4`；桌面 SQLite v8 / Web v9 | Web 离线 `499 passed, 25 skipped, 2 deselected`，`-W error` 零告警；严格日历、最多五次重试、认证 outcomes API 与迁移门全绿 | 真实下一交易日 09:45/14:45 同点结算仍待现场验收 | `local_code_complete_offline_verified_after_alpha4_reliability_fix` |
...
| Web | `fix/web-top3-gain-emphasis@cfc6cd6` | `499 passed, 25 skipped, 2 deselected`、`-W error` 零告警；依赖审计无已知漏洞；Schema v9；Mac Docker `web-alpha4-cfc6cd6`、Web/Worker/Tunnel、公网 CSS 与备份完整性验证通过 | 真实交易日 09:45/14:45 固定提醒、同点结算/重试与全日运行现场验收 | **`BLOCKED / NOT_ACCEPTED`** |
