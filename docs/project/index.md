...
- 当前用户 / 场景：Human Owner 在 Windows 本机内部使用；当前 V1 只做 Windows。
...
V1 已实现 `domain`、`providers`、`runtime`、`engine`、`storage` 和 Windows `ui` 主链路，
并提供不冒充盘中证据的真实收盘回顾；真实交易时段和安装实机证据尚未完成。
