...
先停止重复触发并等待供应商普通 Pro 限流恢复；恢复后继续使用系统钥匙串中的 Primary 凭据、
文档 SDK 路径和 1 秒共享预算，从 `stock_basic` 失败点恢复，完成 1/100/300/800、连续三轮、
手动真实 Top3 和固定弹窗。若普通 Pro 持续429，需要供应商确认该 Token/代理路由的限额状态，
不得用高级静态诊断、Replay或大模型冒充主路线通过。完整真实数据门通过后，才进入最终
