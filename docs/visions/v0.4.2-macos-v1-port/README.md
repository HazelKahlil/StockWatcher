...
> 状态：进行中（已完成离线实现与系统 Keychain 实测；等待 Human Owner 从应用内输入 Token 后做真实数据门）
>
> 创建：2026-07-30
>
> 冻结业务基线：`5b20b707e83baa16b1486894f8e53f343830d67c`
>
> 共享前置提交：`74b4840d25766097a2c88e502983b375bc80c7d6`
>
...
- [ ] Human Owner 在应用“数据接口”页输入并保存真实 Token；Agent 不索取、不回显、
  不写入命令、配置、日志或截图。
...
- 共享返修仅位于 `74b4840d…`；本分支不修改其历史，也不将 macOS 路径、Keychain、
  生命周期、通知或打包代码回填到 Windows 共享提交。
...
完成本分支离线全量验证与独立 Mac 提交后，由 Human Owner 在应用设置页亲自粘贴 Token；
随后按交易时段实际数据完成 1/100/300/800、Top3、固定提醒和恢复证据，再进入 `.app`
构建与最终交接包。
