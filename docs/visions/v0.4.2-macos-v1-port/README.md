...
> 状态：进行中（已完成离线实现、系统 Keychain、原子静态缓存层和实时/Pro 分离；Primary
> 凭据已安全保存，交易时段单只原生实时已再次通过，普通 Pro 仍限流，尚无全市场 Top3）
>
> 创建：2026-07-30
>
...
- 2026-07-30：单实例 guard 已覆盖异常中断后遗留 Unix socket 的恢复：仅在
  ...
  唤起，避免第二次启动永久失败。
- 2026-07-31：新增手动 Top3、固定提醒回归、完整盘后回顾、PDF下载、60秒失败重试、原子
  静态缓存和实时/Pro分离后，离线门重新通过：`uv sync --all-groups --frozen`、
  `uv lock --check`、全量 pytest（284 passed、20 skipped、2 deselected）、Ruff、
  Mypy（98 个源文件）、workspace 和 `git diff --check`；离屏 Replay 五状态 PNG 5/5
  重新生成，SQLite WAL/备份/回滚/迁移定向回归 5/5 通过。视觉复核确认主界面候选区仍为
  主体，“立即获取最新3只”为左侧宽主按钮。
...
等待供应商普通 Pro 限流恢复后，使用系统钥匙串中的 Primary、文档 SDK 路径和1秒共享预算
一次生成完整原子静态缓存；随后交易窗口只运行1/100/300/800原生实时递进、全市场七批和
连续三轮，完成手动真实Top3与固定弹窗。若普通Pro持续429，需要供应商确认该Token/代理路由
的限额状态，不得用高级静态诊断、Replay、大模型或单只实时成功冒充主路线通过。完整真实
数据门通过后，才进入最终PyInstaller `.app`构建与最终交接包。
