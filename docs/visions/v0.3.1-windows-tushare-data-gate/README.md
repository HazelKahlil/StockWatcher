...
Tushare 静态、日线、三日历史分钟和板块能力真实可用；Super `rt_k` 的首次盘中 M0
仍为 `FAIL`，首个严格失败报告必须保留。经明确授权的原生实时路线已通过盘后全市场
吞吐和量额单位核对，但尚未通过交易时段连续 M0，因此真实候选、09:45/14:50 和盘中
特别强异动提醒保持关闭。紫黄线、Level-2 继续为 `unavailable`。
