...
在 Mac 内部试用继续开发中，跨平台共享层补齐了交接 Brief 指出的运行缺口：

- 基础缓存准备成功时合并完整 `tdx_index` / `tdx_member` 概念关系；行业和概念仍由同一个
  本地 `SectorEngine` 计算，任一板块通过即可成为正式候选，关系不在实时轮次逐只请求。
- 候选引擎暴露全量正式候选池，`StrongMovementDetector` 默认只比较得分前 100 名；补位
  `近` 不会触发强异动，实时弹窗仍使用稳定三只。
- `medium_core_score` 现在真正参与等级判定；正式候选只有达到中级核心分才显示“中”，其余
  保持“近”，强级门仍保留完整度和加速度约束。
- 稳定器新增可配置的最短席位保持时间，并允许强异动突破普通保持门；实时运行传入扫描完成
  时间，回放调用不注入时钟则保持原有确定性行为。
- 会话按本地日期调度一次 `prune_history()`，实际清理超过30天的提醒及无引用快照；清理
  失败只显示维护提示，不阻塞实时候选。

本增量只修改共享 engine/runtime/session 逻辑和确定性测试；不改变实时供应商路线、资金边界或
Windows 真实验收结论。
