...
状态：核心源码交接，等待完整项目集成、审查和用户视觉验收；未授权生产迁移/上线。

适配源码基准：`7e103f6249a215de8f90e74e4291380853190788`（Web 独立线）。这不是包含新功能的最终提交号，接入后需记录实际 SHA。
...
- [ ] 已回读实际项目新增代码和六个接入点，无未审查的本地覆盖。
- [ ] Python 3.12 锁定环境新增存储/API/真实 app 测试通过。
- [ ] Node、现有 UI 回归、全量 pytest、Ruff、Mypy 与工作区检查通过。
- [ ] 开启功能的正式页面通过浏览器交互与几何验收，而不只是默认关闭的旧 harness。
- [ ] 临时库认可、重载、两账户、撤销、快照切换、超时、并发及清理后上下文保留通过。
- [ ] 用户认可右侧控件视觉与操作。
- [ ] 备份、扩展迁移、关闭开关/旧镜像兼容在隔离环境验证。
