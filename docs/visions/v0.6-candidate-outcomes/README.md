...
> 状态：`local_code_complete_offline_verified_after_alpha4_reliability_fix`；GitHub 里程碑同步为 PR #6，真实交易时段同点行情仍待 Human Owner 验收。
...
不覆盖已安装 App。GitHub 只做用户授权的代码里程碑同步，不发布 tag/Release、不修改部署。非交易时段的代码、回放、迁移和 UI
