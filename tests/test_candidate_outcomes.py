...
def test_v7_to_v9_migration_persists_outcome_retry_state_and_backup(tmp_path: Path) -> None:
    path = tmp_path / "v7-to-v9.sqlite3"
    with closing(sqlite3.connect(path)) as connection, connection:
        connection.execute(
            "CREATE TABLE schema_version (version INTEGER NOT NULL, applied_at TEXT NOT NULL)"
        )
...
def test_v9_migration_failure_rolls_back_and_releases_handles(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    path = tmp_path / "v9-failure.sqlite3"
    with closing(sqlite3.connect(path)) as connection, connection:
        connection.execute(
            "CREATE TABLE schema_version (version INTEGER NOT NULL, applied_at TEXT NOT NULL)"
        )
        connection.execute("INSERT INTO schema_version VALUES (8, '2026-08-11T09:00:00+08:00')")
        SQLiteStore._apply_v1_schema(connection)
        SQLiteStore._apply_v2_migration(connection)
        SQLiteStore._apply_v3_migration(connection)
        SQLiteStore._apply_v4_migration(connection)
        ...
        SQLiteStore._apply_v6_migration(connection)
        SQLiteStore._apply_v7_migration(connection)
        SQLiteStore._apply_v8_migration(connection)

    def fail_v9(connection: sqlite3.Connection) -> None:
        connection.execute("CREATE TABLE partial_outcome_table (id INTEGER PRIMARY KEY)")
        raise RuntimeError("injected v9 failure")

    monkeypatch.setattr(SQLiteStore, "_apply_v9_migration", staticmethod(fail_v9))
    store = SQLiteStore(path)
    ...
    with closing(sqlite3.connect(path)) as connection:
        ...
        assert connection.execute("PRAGMA integrity_check").fetchone() == ("ok",)
    assert path.with_suffix(".sqlite3.pre-v9.bak").exists()
    path.replace(tmp_path / "v9-failure-renamed.sqlite3")


def test_v8_to_v9_migration_preserves_web_accounts_sessions_commands_and_events(
    tmp_path: Path,
) -> None:
    path = tmp_path / "web-v8.sqlite3"
    at = "2026-08-11T09:00:00+08:00"
    with closing(sqlite3.connect(path)) as connection, connection:
        connection.execute(
            "CREATE TABLE schema_version (version INTEGER NOT NULL, applied_at TEXT NOT NULL)"
        )
        connection.execute("INSERT INTO schema_version VALUES (8, ?)", (at,))
        for migration in (
            SQLiteStore._apply_v1_schema,
            SQLiteStore._apply_v2_migration,
            SQLiteStore._apply_v3_migration,
            SQLiteStore._apply_v4_migration,
            SQLiteStore._apply_v5_migration,
            SQLiteStore._apply_v6_migration,
            SQLiteStore._apply_v7_migration,
            SQLiteStore._apply_v8_migration,
        ):
            migration(connection)
        connection.execute(
            "INSERT INTO web_users "
            "(user_id, username, password_hash, role, active, created_at, updated_at, "
            "password_changed_at) VALUES (1, 'tester', 'hash', 'tester', 1, ?, ?, ?)",
            (at, at, at),
        )
        connection.execute(
            "INSERT INTO web_sessions "
            "(session_token_hash, user_id, csrf_token_hash, created_at, last_seen_at, "
            "idle_expires_at, absolute_expires_at) VALUES (?, 1, ?, ?, ?, ?, ?)",
            ("a" * 64, "b" * 64, at, at, at, at),
        )
        connection.execute(
            "INSERT INTO web_commands "
            "(command_id, command_type, status, requested_by, requested_at) "
            "VALUES ('command-1', 'manual_refresh', 'queued', 1, ?)",
            (at,),
        )
        connection.execute(
            "INSERT INTO web_events "
            "(event_type, occurred_at, source_commit, payload_json) "
            "VALUES ('state.changed', ?, 'commit', '{}')",
            (at,),
        )
    ...
    with closing(sqlite3.connect(path)) as connection:
        ...
        assert connection.execute("SELECT event_type FROM web_events").fetchone() == (
            "state.changed",
        )
        assert connection.execute("PRAGMA integrity_check").fetchone() == ("ok",)
    assert path.with_suffix(".sqlite3.pre-v9.bak").exists()
