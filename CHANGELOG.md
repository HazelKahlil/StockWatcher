...
## [0.7.0-alpha.3] - 2026-09-06

- Web 添加可打断的详情与折叠过渡、页面切换反馈，尊重减少动态效果设置。
- 两端复用候选控件与焦点；Web 按展开日期创建复盘明细，Mac 历史与复盘分批显示。
- Mac 手动面板改为异步打开，报告读取、PDF 准备和保存移到后台，关闭不再等待历史读取。
- 验证与发布状态见 `docs/visions/v0.7-ui-motion/README.md`。

## [0.7.0-alpha.2] - 2026-09-06

- 保留浅底白卡与蓝色操作，统一导航选中态、字号、间距、表单焦点和窄屏布局。
- 首页分别展示候选源时间与扫描时间，明确保留快照和周末休市；候选详情支持加载、重试、Esc 与焦点返回。
- 次日复盘按日期折叠明细，增加股票与结算状态筛选，保留原排名、统计分母和完整六笔日组合规则。
- 历史支持 Enter 筛选与重置；总结支持直接阅读正文与下载失败反馈；技术明细默认收起。
- Mac 增加候选卡键盘操作与可见焦点，两端共用回补状态文案，不再把跳过数量表述成未结算数量。
- 本地隔离验证与配对候选详见 `docs/visions/v0.7-ui-refinement/README.md`；真实行情验收边界不变。

## [0.7.0-alpha.1] - 2026-09-05

- Mac 与 Web 使用同一产品版本来源；页面、App、快照和打包元数据同步，新增轻量版本查询。
- Mac 接入共用的重复出现规则，在强异动弹窗与历史显示提示，固定提醒和主页不加紫色。
- Mac 固定提醒账本保留已送达状态与首次失败时间；15:30 总结防止并发重复执行。
- Web 轻量健康检查进入镜像源码，避免 15 秒一次的全库完整性扫描。
- 配对构建只接受干净提交，从同一 Git archive 生成 Mac App / Web 镜像并记录哈希。
- 同提交构建与 Mac/Web 模拟复盘界面核对通过，Owner 回复“继续”后已完成 Mac 正式安装、
  Web/Worker 同镜像切换及公网恢复；升级前后备份均保留。Chrome 真实登录后的看板、复盘、总结及 09-04 PDF 已读回。
  具体记录见 `docs/visions/v0.7-mac-web-sync/README.md`。Windows 不在本次包内。
