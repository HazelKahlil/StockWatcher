...
def main() -> int:
    ...
    if (
        "StockWatcher-0.6.0-alpha.3" not in powershell
        or "StockWatcher-0.6.0-alpha-" in powershell
        or "0.3.1-alpha" in powershell
    ):
        errors.append(
            "PowerShell build must publish only the current 0.6.0-alpha.3 artifacts"
        )
    ...
    if (
        '#define MyAppVersion "0.6.0-alpha.3"' not in installer
        or "StockWatcher-0.6.0-alpha.3-setup" not in installer
    ):
        errors.append(
            "installer must identify the candidate outcome build as 0.6.0-alpha.3"
        )
